GF2_Subtitles_Table = GF2_Subtitles_Table or {} -- don't touch

local CharacterName = "米什缇："
local CharacterColor = Color(70,140,135,255)
local Language = "schinese2"

local GF2Subtitle = { -- add new tables inside here
	
	{
		snd = "vo/jp/mechty/alert1.wav", 
		subject = CharacterName, 
		text = "好！", 
		subjectcol = CharacterColor,
		lang = Language,
	},
	
	{
		snd = "vo/jp/mechty/alert2.wav",
		subject = CharacterName,
		text = "锵锵，现在登场的就是本世纪最伟大的天才，万能人形奇塔。",
		subjectcol = CharacterColor,
		lang = Language,
	},

	{
		snd = "vo/jp/mechty/alert3.wav",
		subject = CharacterName,
		text = "还要来啊？",
		subjectcol = CharacterColor,
		lang = Language,
	},

	{
		snd = "vo/jp/mechty/alert4.wav",
		subject = CharacterName,
		text = "差不多得了吧？",
		subjectcol = CharacterColor,
		lang = Language,
	},

	{
		snd = "vo/jp/mechty/die1.wav",
		subject = CharacterName,
		text = "失、失败了？！",
		subjectcol = CharacterColor,
		lang = Language,
	},

	{
		snd = "vo/jp/mechty/die2.wav",
		subject = CharacterName,
		text = "一定是，是情报出错了！",
		subjectcol = CharacterColor,
		lang = Language,
	},

	{
		snd = "vo/jp/mechty/die3.wav",
		subject = CharacterName,
		text = "失、失败了？！一定是，是情报出错了！",
		subjectcol = CharacterColor,
		lang = Language,
	},

	{
		snd = "vo/jp/mechty/follow1.wav",
		subject = CharacterName,
		text = "嗯嗯。",
		subjectcol = CharacterColor,
		lang = Language,
	},

	{
		snd = "vo/jp/mechty/follow2.wav",
		subject = CharacterName,
		text = "哈！",
		subjectcol = CharacterColor,
		lang = Language,
	},

	{
		snd = "vo/jp/mechty/follow3.wav",
		subject = CharacterName,
		text = "哦，在看什么呢？",
		subjectcol = CharacterColor,
		lang = Language,
	},

	{
		snd = "vo/jp/mechty/follow4.wav",
		subject = CharacterName,
		text = "你好，饲养员！",
		subjectcol = CharacterColor,
		lang = Language,
	},

	{
		snd = "vo/jp/mechty/follow5.wav",
		subject = CharacterName,
		text = "好好好。",
		subjectcol = CharacterColor,
		lang = Language,
	},

	{
		snd = "vo/jp/mechty/get_heal1.wav",
		subject = CharacterName,
		text = "嗯...一般般吧。",
		subjectcol = CharacterColor,
		lang = Language,
	},

	{
		snd = "vo/jp/mechty/get_heal2.wav",
		subject = CharacterName,
		text = "还、还有吗？全都给我！",
		subjectcol = CharacterColor,
		lang = Language,
	},

	{
		snd = "vo/jp/mechty/get_heal3.wav",
		subject = CharacterName,
		text = "噢！还不错嘛！",
		subjectcol = CharacterColor,
		lang = Language,
	},

	{
		snd = "vo/jp/mechty/get_heal4.wav",
		subject = CharacterName,
		text = "还不够还不够！",
		subjectcol = CharacterColor,
		lang = Language,
	},

	{
		snd = "vo/jp/mechty/get_heal5.wav",
		subject = CharacterName,
		text = "现在，本天才是完全形态啦！",
		subjectcol = CharacterColor,
		lang = Language,
	},

	{
		snd = "vo/jp/mechty/grenade_attack1.wav",
		subject = CharacterName,
		text = "耶耶！",
		subjectcol = CharacterColor,
		lang = Language,
	},

	{
		snd = "vo/jp/mechty/grenade_attack2.wav",
		subject = CharacterName,
		text = "啧啧啧。",
		subjectcol = CharacterColor,
		lang = Language,
	},

	{
		snd = "vo/jp/mechty/grenade_attack3.wav",
		subject = CharacterName,
		text = "变成碎片吧！",
		subjectcol = CharacterColor,
		lang = Language,
	},

	{
		snd = "vo/jp/mechty/grenade_attack4.wav",
		subject = CharacterName,
		text = "看我看我！",
		subjectcol = CharacterColor,
		lang = Language,
	},

	{
		snd = "vo/jp/mechty/grenade_attack5.wav",
		subject = CharacterName,
		text = "笨蛋！",
		subjectcol = CharacterColor,
		lang = Language,
	},

	{
		snd = "vo/jp/mechty/grenade_attack6.wav",
		subject = CharacterName,
		text = "这里这里！",
		subjectcol = CharacterColor,
		lang = Language,
	},

	{
		snd = "vo/jp/mechty/grenade_attack7.wav",
		subject = CharacterName,
		text = "差不多行了！",
		subjectcol = CharacterColor,
		lang = Language,
	},

	{
		snd = "vo/jp/mechty/hit1.wav",
		subject = CharacterName,
		text = "啊！",
		subjectcol = CharacterColor,
		lang = Language,
	},

	{
		snd = "vo/jp/mechty/hit2.wav",
		subject = CharacterName,
		text = "切！",
		subjectcol = CharacterColor,
		lang = Language,
	},

	{
		snd = "vo/jp/mechty/hit3.wav",
		subject = CharacterName,
		text = "真无聊...",
		subjectcol = CharacterColor,
		lang = Language,
	},

	{
		snd = "vo/jp/mechty/hit4.wav",
		subject = CharacterName,
		text = "切，碍眼。",
		subjectcol = CharacterColor,
		lang = Language,
	},

	{
		snd = "vo/jp/mechty/hit5.wav",
		subject = CharacterName,
		text = "麻烦。",
		subjectcol = CharacterColor,
		lang = Language,
	},

	{
		snd = "vo/jp/mechty/idle1.wav",
		subject = CharacterName,
		text = "哦，这不是饲养员吗？今天也一副工作好烦好想睡觉的样子呢，哈哈哈。",
		subjectcol = CharacterColor,
		lang = Language,
	},

	{
		snd = "vo/jp/mechty/idle2.wav",
		subject = CharacterName,
		text = "不许盯着我看！变这么矮又不是我愿意的...都怪饲养员的饲养员那家伙！",
		subjectcol = CharacterColor,
		lang = Language,
	},

	{
		snd = "vo/jp/mechty/idle3.wav",
		subject = CharacterName,
		text = "饲养员有没有见到过蜜罐啊？我、我也不是很在意啦，就是...那家伙可不能先输给别人啊。",
		subjectcol = CharacterColor,
		lang = Language,
	},

	{
		snd = "vo/jp/mechty/idle4.wav",
		subject = CharacterName,
		text = "饲养员，我想要最新的电子游戏，给我买。啊？为啥不行啊？饲养员不就是要照看身为天才的奇塔吗？",
		subjectcol = CharacterColor,
		lang = Language,
	},

	{
		snd = "vo/jp/mechty/idle5.wav",
		subject = CharacterName,
		text = "唉...跟队长他们开视频会议好累啊，下次开会干脆让我的傀儡坐在屏幕前就好了，再给它加上不管被问到什么都会回答“啊对对对”的功能。",
		subjectcol = CharacterColor,
		lang = Language,
	},

	{
		snd = "vo/jp/mechty/idle6.wav",
		subject = CharacterName,
		text = "上次作战我把无人机打下来的事情，饲养员可不要告诉饲养员的饲养员啊，嗯被她知道的话又要啰嗦了。",
		subjectcol = CharacterColor,
		lang = Language,
	},

	{
		snd = "vo/jp/mechty/idle7.wav",
		subject = CharacterName,
		text = "饲养员饲养员，托洛洛那家伙又不许我进她房间了，好过分啊。嗯，天才，果然都是孤独的呢。",
		subjectcol = CharacterColor,
		lang = Language,
	},

	{
		snd = "vo/jp/mechty/idle8.wav",
		subject = CharacterName,
		text = "切，不就是给引擎加了个超过一定转速就能尖叫的警报嘛，美玲怎么也这么啰嗦。好好好，我知道了，下次一定注意，可以了吧？",
		subjectcol = CharacterColor,
		lang = Language,
	},

	{
		snd = "vo/jp/mechty/idle9.wav",
		subject = CharacterName,
		text = "哈啰，饲养员，嗯，这个反应...忘了我的新名字？奇塔啦，奇塔，那个会吃蜜罐的猎豹，啊呜...啊不对，叫声应该是...啾啾，嘿嘿。",
		subjectcol = CharacterColor,
		lang = Language,
	},

	{
		snd = "vo/jp/mechty/idle10.wav",
		subject = CharacterName,
		text = "好，这样就准备万全了。我接下来要进行一个非常厉害的实验，这是战胜蜜罐的重要一步，所以饲养员也要来帮忙。",
		subjectcol = CharacterColor,
		lang = Language,
	},

	{
		snd = "vo/jp/mechty/killed_enemy1.wav",
		subject = CharacterName,
		text = "呀哈！",
		subjectcol = CharacterColor,
		lang = Language,
	},

	{
		snd = "vo/jp/mechty/killed_enemy2.wav",
		subject = CharacterName,
		text = "拜拜！",
		subjectcol = CharacterColor,
		lang = Language,
	},

	{
		snd = "vo/jp/mechty/killed_enemy3.wav",
		subject = CharacterName,
		text = "哼哼！有我在，就没什么好担心的了！",
		subjectcol = CharacterColor,
		lang = Language,
	},

	{
		snd = "vo/jp/mechty/killed_enemy4.wav",
		subject = CharacterName,
		text = "哼！蜜罐做得到吗？",
		subjectcol = CharacterColor,
		lang = Language,
	},

	{
		snd = "vo/jp/mechty/unfollow1.wav",
		subject = CharacterName,
		text = "唉？",
		subjectcol = CharacterColor,
		lang = Language,
	},

	{
		snd = "vo/jp/mechty/unfollow2.wav",
		subject = CharacterName,
		text = "真是的...",
		subjectcol = CharacterColor,
		lang = Language,
	},

	{
		snd = "vo/jp/mechty/unfollow3.wav",
		subject = CharacterName,
		text = "啊？",
		subjectcol = CharacterColor,
		lang = Language,
	},

	{
		snd = "vo/jp/mechty/player_sight1.wav",
		subject = CharacterName,
		text = "喂，饲养员的眼神...您在想“哇，好矮啊！”对吧？我都看出来了！",
		subjectcol = CharacterColor,
		lang = Language,
	},

	{
		snd = "vo/jp/mechty/player_sight2.wav",
		subject = CharacterName,
		text = "唉，战术什么的无所谓吧，反正我这种天才是不会输的啦。嗯？好好好，我会去看作战资料的，真的。",
		subjectcol = CharacterColor,
		lang = Language,
	},

	{
		snd = "vo/jp/mechty/player_sight3.wav",
		subject = CharacterName,
		text = "啊，饲养员，在看什么呢，是不是《一百个创意发明大解析》？我也要看，让我也看看！",
		subjectcol = CharacterColor,
		lang = Language,
	},

	{
		snd = "vo/jp/mechty/player_sight4.wav",
		subject = CharacterName,
		text = "饲养员别睡了，快起来陪我玩游戏，快起来啦。唉，真是的，人类的身体几乎每天都要休眠，真麻烦，唉，不如让我来做个小道具吧。",
		subjectcol = CharacterColor,
		lang = Language,
	},

	{
		snd = "vo/jp/mechty/player_sight5.wav",
		subject = CharacterName,
		text = "饲养员，您在干什么？呵呵，不会是背着我偷偷练习上次那个赛车游戏吧？别天真了，我这样的天才才不会输呢。",
		subjectcol = CharacterColor,
		lang = Language,
	},

	{
		snd = "vo/jp/mechty/player_sight6.wav",
		subject = CharacterName,
		text = "那个，饲养员，我们关系很好对吧？我们是朋友对吧？嗯，那我刚刚把仓库炸掉了，饲养员应该也不会生气的对吧？",
		subjectcol = CharacterColor,
		lang = Language,
	},

	{
		snd = "vo/jp/mechty/player_sight7.wav",
		subject = CharacterName,
		text = "额...不想上班，嗯？干脆翘掉吧，啊，可是今天有任务安排啊。唉，要不饲养员替我去吧，好不好，好不好嘛？",
		subjectcol = CharacterColor,
		lang = Language,
	},

	{
		snd = "vo/jp/mechty/player_sight8.wav",
		subject = CharacterName,
		text = "我的新发明即将闪亮登场了，饲养员，期不期待，期不期待？诶呀，尽情期待，然后来崇拜我吧。",
		subjectcol = CharacterColor,
		lang = Language,
	},

	{
		snd = "vo/jp/mechty/player_sight9.wav",
		subject = CharacterName,
		text = "我说，之前那个素体还有现在这个素体，饲养员更喜欢哪个啊？啊，别在意，我只是随便问问，反正我都是要换回去的。",
		subjectcol = CharacterColor,
		lang = Language,
	},

	{
		snd = "vo/jp/mechty/player_sight10.wav",
		subject = CharacterName,
		text = "饲养员，我的新发明，“睁眼拟态器”快完成啦。这样饲养员就算困到闭着眼睛工作，只要戴上它，在别人眼里也是睁着眼睛的，很厉害吧。",
		subjectcol = CharacterColor,
		lang = Language,
	},

	{
		snd = "vo/jp/mechty/player_sight11.wav",
		subject = CharacterName,
		text = "饲养员，工作还要做多久啊，先陪我打一局《特工精英》吧，大不了我等一下陪你工作嘛。",
		subjectcol = CharacterColor,
		lang = Language,
	},

	{
		snd = "vo/jp/mechty/player_sight12.wav",
		subject = CharacterName,
		text = "锵锵，现在登场的就是本世纪最伟大的天才，万能人形奇塔。嘿嘿，你有什么愿望我都会帮你...额，尽力实现。",
		subjectcol = CharacterColor,
		lang = Language,
	},

	{
		snd = "vo/jp/mechty/player_sight13.wav",
		subject = CharacterName,
		text = "您看最新的作战报告了吗？一下子炸掉整个作战区域的我，是不是超厉害？啊？差点也炸到基地车，哎呀，那种小事就别在意了。",
		subjectcol = CharacterColor,
		lang = Language,
	},

}

table.insert(GF2_Subtitles_Table,GF2Subtitle) -- don't touch