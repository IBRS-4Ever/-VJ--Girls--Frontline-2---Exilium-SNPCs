...
include('shared.lua')
ENT.Model = "models/prop/gfl2_ledyanaya_mogila_cannon.mdl"
util.PrecacheModel(ENT.Model)
ENT.StartHealth = 25000
...
ENT.RangeAttackMaxDistance = 30000
...
function ENT:OnRangeAttackExecute(status, enemy, projectile)
	...
	if status == "Init" then
					...
					for id, ent in pairs( ents.FindInSphere( HitPos, 2048 * GetConVar("vj_gf2_npc_element_electric_radius_multipler"):GetFloat() ) ) do
						...
						if ent:IsNPC() or ent:IsPlayer() and ent:Alive() then
							if self:CheckRelationship(ent) == D_HT then
								...
								DmgInfo:SetDamage( 15000 * GetConVar("vj_gf2_npc_element_electric_damage_multipler"):GetFloat()  )
		...
		VJ.EmitSound(self, "sfx/deep_warship/cannon_charge.wav", 100, math.random(100, 110))
		...
		timer.Simple(7.2, function() if IsValid(self) then self:StopParticles() end end)
